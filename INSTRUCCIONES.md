...
| `update_tools.py` | Avisa de forma **visible** (`::error::`/`::warning::`) si falla la clave de IA (OpenRouter) o los feeds. Pide a IA (OpenRouter) fichas completas (con ética). Lo que entre incompleto se marca `pendiente_revision` y no contamina la sección ética. |
...
- `ITACA` → tu API key de OpenRouter (imprescindible para detectar bien).
...
2. Ejecútalo con `modo_prueba = 0`. Si la clave IA (OpenRouter) está bien, en el log verás las herramientas detectadas; si algo falla, saldrá un aviso rojo/amarillo (ya no se queda "verde en silencio").
