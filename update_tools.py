...
# ─── Scraping de directorios de herramientas ─────────────────
print("Scraping directorios...")
try:
 import requests
 dirs=[
 ...
 for d in dirs:
  try:
   resp=requests.get(d,timeout=15,headers={"User-Agent":"ViajeAItaca/1.0"})
   if resp.status_code==200:
    found=re.findall(r'title=["\']([^"\']{3,60})["\']',resp.text,re.IGNORECASE)
    found+=re.findall(r'<h[23][^>]*>([^<]{3,60})</h[23]>',resp.text,re.IGNORECASE)
    for nm in found[:15]:
     nm=nm.strip()
     if 3<len(nm)<50 and nm.lower() not in [y.lower() for y in nombres] and not re.search(r'<[^>]+>',nm):
      if not any(cx.get("nombre","")==nm for cx in c):
       c.append({"nombre":nm,"ti":f"Encontrado en {d}","re":"","li":d})
    print(f"  {d[:40]}: {len(found)} nombres")
  except Exception as ex:print(f"  Error: {ex}")
except Exception as ex:print(f"Scraping error: {ex}")
